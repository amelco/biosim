...
do while (eof .ne. -1)
  ...
  ! calc. root growth 
	if (i .eq. 1) then
	  ...
	  root_depth = root_depth + rootGrowth(reduction_fac(Wi_ant(1), Wz, beta), dBr)
  endif
...
subroutine calcWz(Or1, Os1, z1, Or2, Os2, root_depth, Wz)
  ...
  real, intent(out) :: Wz
	...
	if (root_depth > z1) then
	  Wz = ((Os1 - Or1) * z1) + ((Os1 - Or1) * (root_depth - z1))
	else
...
subroutine tempSWB(layer, water_in)
  ...
  if (layer .eq. 1) then
    !ETa = ((Wi_ant(layer) / Wx(layer))**beta) * ET_ant
		! OBS.: ETa is calculated for the top layer only !!!!!!!!!
		! To do: Extract water from bottom layer when top layer is dry
    ETa = reduction_fac(Wi_ant(layer),  Wz, beta) * ET_ant
...
real function reduction_fac(Wi, Wz, beta)
  implicit none
	real, intent(in) :: Wi, Wz, beta

	reduction_fac = (Wi/Wz)**beta
...
real function LAperShoot2(rho, Am, Ant, tau, Wi, Wz, beta)
  implicit none
  real, intent(in) :: rho, Am, Ant, tau, Wi, Wz, beta
  real :: dA
	real, parameter :: afac = 1.0

  dA = rho * (Am - Ant) / tau - afac * (1.0 - reduction_fac(Wi, Wz, beta)) * Ant
  LAperShoot2 = Ant + dA
  return 
end function
...
implicit none
  real, intent(in) :: omega, dBr
	...
	!print*, omega, rdg, Broot, rootGrowth
	!read*
	return
end function
...
110 FORMAT (2I7,11(F8.2,2x)) 
  ...
  if (eof .ne. -1) then
    ...
    if (int(Year/4.0)*4 == Year) then
      if (SQD == 366) then
        write(*,110) Year, SQD, ET, Eta, ETa/ET_ant, Wi(1), Wi(2), Wx(1), Wx(2), D, water_in, BL, reduction_fac(Wi(1), Wz, beta)
        Dsum      = 0
        rainsum   = 0
        BLsum     = 0
        ETavg     = 0
    ...
    write(1,110) Year, SQD, ET, Eta, ETa/ET_ant, Wi(1), Wi(2), Wx(1), Wx(2), D, water_in, BL, reduction_fac(Wi(1), Wz, beta)
